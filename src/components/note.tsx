...
export default function MinimalistNotepad(): JSX.Element {
  ...
  const handleAddItem = useCallback((e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    if (newItemText.trim()) {
  ...
  async function handleFetchData(docid:string) {
    try {
      const response = await fetch(
        `http://localhost:8000/fetch/${docid}`);
      const data = await response.json();
      ...
      <header className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8 flex justify-between items-center">
            <div className=' flex items-center space-x-5'>
                ...
                <h1 className="text-lg font-bold text-gray-900">Minimalist Notepad</h1>
            </div>
